...
// 2. refundedTickets (billets remboursés)
// Cela désigne le nombre de billets qui ont été retournés par les acheteurs et pour lesquels un remboursement a été effectué. Ces billets ne sont plus valides et ne peuvent pas être utilisés pour l'événement.

// 3. cancelledTickets (billets annulés)
// Cela fait référence aux billets qui ont été annulés, soit par l'organisateur de l'événement, soit par l'acheteur. Ces billets ne peuvent pas être utilisés et ne seront pas remboursés.

// 4. revenue (revenu)
// Cela désigne le montant total d'argent généré par la vente de billets pour un événement. Il s'agit souvent d'un indicateur clé de la performance financière de l'événement.